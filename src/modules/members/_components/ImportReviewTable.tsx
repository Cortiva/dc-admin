...
import { COLUMN_HEADERS, validateMemberRow, MEMBER_IMPORT_COLUMNS, type RawImportRow } from "../memberImportValidation";
...
// Primary columns shown directly
...
export function ImportReviewTable({ rows, onChangeCell, onRemoveRow }: ImportReviewTableProps) {
    const rowsWithErrors = useMemo(
        () =>
            rows.map((r) => ({
                ...r,
                errors: validateMemberRow(r.data),
            })),
        [rows],
    );
            ...
            <Table>
                ...
                <TableBody>
                    {rowsWithErrors.map((row, index) => {
                        ...
                        const secondaryColumns = MEMBER_IMPORT_COLUMNS.filter(
